...
from personnages.Boss import Boss
...
class Monde:
	...
	def __init__(self):
		...
		self.time = 90
		self.solAbsolu = 285
		self.couleurPieces = (0,0,0)
		self.couleurTime  = (255,0,250)
		self.couleurVieMario = (0,255,250)
		#self.peupler(nomMonde)
	...
	def getCouleurPieces(self):
		return self.couleurPieces
	def getCouleurTime(self):
		return self.couleurTime
	def getCouleurVieMario(self):
		return self.couleurVieMario
	#Setters
	def setDx(self,dx):
		self.dx = dx
	...
	def peupler(self,monde):
		for fichier in self.tabObjetsTxt:
			...
			for i in text.split():
				cordsTab.append(i)
				if fichier == "Positionnements/"+monde+"/sol.txt":
			...
			for x,y in tabTuples:
				if fichier == "Positionnements/"+monde+"/murs.txt":
					self.tabObjets.append(Murs(int(x),int(y)))
				if fichier == "Positionnements/"+monde+"/tuyau.txt":
					self.tabObjets.append(TuyauRouge(int(x),int(y)))
				if fichier == "Positionnements/"+monde+"/bloc.txt":
					self.tabObjets.append(Bloc(int(x),int(y)))
				if fichier == "Positionnements/"+monde+"/bloc_nazo.txt":
					self.tabObjets.append(Bloc_Nazo(int(x),int(y)))
				if fichier == "Positionnements/"+monde+"/trou.txt":
					self.tabObjets.append(Trou(int(x) ,int(y),monde ))
		#on ouvre le fichier piece.txt contenant les Coordonnées des pièces sur l'écran
		fr = open("Positionnements/"+monde+"/piece.txt",'r')
		text = fr.read()
		...
		for fichier in self.tabPersTxt:
			...
			tabTuples =  zip(cordsTab[0::2], cordsTab[1::2])
			for x,y in tabTuples:
				if fichier == "Positionnements/"+monde+"/champignon.txt":
					self.tabPersonnages.append(Champignon(int(x) ,int(y)))
				if fichier == "Positionnements/"+monde+"/tortue.txt":
					self.tabPersonnages.append(Tortue(int(x) ,int(y)))
				if fichier == "Positionnements/"+monde+"/plante.txt":
...
class Monde_1(Monde):
		def __init__(self):
			...
			self.peupler("Monde_1")
class Monde_2(Monde):
		def __init__(self):
			super().__init__()
			self.strImgFond = "Images/Monde_2/fondEcran.png"
			...
			self.couleurPieces = (255,0,0)
			self.couleurTime  = (255,140,65)
			self.couleurVieMario = (255,140,165)
			self.peupler("Monde_2")
class Monde_3(Monde):
		def __init__(self):
			super().__init__()
			self.strImgFond = "Images/Monde_3/fondEcran.png"
			self.tabObjetsTxt = ["Positionnements/Monde_3/murs.txt","Positionnements/Monde_3/tuyau.txt","Positionnements/Monde_3/bloc.txt","Positionnements/Monde_3/bloc_nazo.txt","Positionnements/Monde_3/trou.txt","Positionnements/Monde_3/sol.txt"]
			self.tabPersTxt = ["Positionnements/Monde_3/champignon.txt","Positionnements/Monde_3/tortue.txt","Positionnements/Monde_3/plante.txt"]
			self.boss = Boss(4400,240,"/Images/Monde_3/bossGauche.png")
			self.couleurPieces = (0,0,0)
			self.couleurTime  = (12,80,200)
			self.couleurVieMario = (0,255,250)
			self.peupler("Monde_3")
		def getBoss(self):
			return self.boss
...
class  Main:
	...
	def transitionner(self):
		if self.transition:
			self.monde = self.determinerMonde(self.monde)
			self.mario = self.transitionnerMario(self.mario,0,0,True)
			#self.mario.setY()
	...
	def determinerMonde(self,monde):
		if isinstance(monde,Monde_1):
			return Monde_1()
		if isinstance(monde,Monde_2):
			return Monde_2()
		if isinstance(monde,Monde_3):
			return Monde_3()
	def transitionnerMonde(self,monde):
		if isinstance(monde,Monde_1):
			return Monde_2()
		if isinstance(monde,Monde_2):
			return Monde_3()
		if isinstance(monde,Monde_3):
			return None
	def transitionnerMario(self,mario,x,y,marioInit):
		if marioInit:
			marioInit = Mario(300,self.monde.getSolAbsolu()-20)
			marioInit.setVieDeMario(self.mario.getVieDeMario())
			marioInit.setPieces(self.mario.getPieces())
			self.mario.playMusik("stop")
			return marioInit
		if isinstance(mario,Mario) and not isinstance(mario,BigMario) and not isinstance(mario,SuperMario):
			bigMario = BigMario(x,y)
			bigMario.setVieDeMario(self.mario.getVieDeMario())
			bigMario.setPieces(self.mario.getPieces())
			return bigMario
		if isinstance(mario,BigMario):
			superMario = SuperMario(x,y)
			superMario.setVieDeMario(self.mario.getVieDeMario())
			superMario.setPieces(self.mario.getPieces())
			return superMario
		if isinstance(mario,SuperMario):
			return None
	...
	def game_Intro(self):
		...
		optionImg = pygame.image.load("Images/Menu/options.png")
		...
		while self.intro:
			...
			if self.regles and not self.languageMenu:
				...
				self.gameFenetre.blit(optionImg,(0,-20))
				y=70
				for tab in tabtext:
			...
			#POUR LE MENU DES LANGUES
				...
				if self.languageMenu and not self.regles:
					self.verifier = True
					tabLangAff.append (-1)
					self.gameFenetre.blit(menuImg,(0,-20))
					i = 0
	...
	def afficher(self):
		score = self.font2.render(self.getHUD(0) + ""+ str(self.mario.getPieces()),True,self.monde.getCouleurPieces())
		tps = self.font1.render(self.getHUD(1) + "" + str(self.monde.getTime()),True,self.monde.getCouleurTime())
		vies = self.font3.render(self.getHUD(2) + ""+ str(self.mario.getVieDeMario()),True,self.monde.getCouleurVieMario())
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				...
				if event.key == pygame.K_RETURN:
					if isinstance(self.mario,SuperMario):
				...
				if event.key == pygame.K_RIGHT:
					...
					if self.monde.getXPos() >= 4400:
						prochainMonde= self.transitionnerMonde(self.monde)
						if not prochainMonde == None:
							self.monde = prochainMonde
							self.mario = Mario(300,monde.getSolAbsolu()-22)
						else:
			...
			if event.type == pygame.KEYUP:
				...
				if event.key == pygame.K_SPACE:
					...
					self.mario.setMarche(False)
	def finJeu(self):
		if self.mario.getY() >= 350 or self.monde.getTime() == 0 or self.mario.getVieDeMario() <= 0:
			self.jeuEnCours = False
	...
	def demarer(self):
		...
		while(self.jeuEnCours):
			...
			for objet in self.monde.getTabObjets():
				...
				for perso in self.monde.getTabPersonnages():
					...
					if isinstance(perso,Magic_Mushroom):
						if self.mario.contactAbsolu(self.mario,perso):
							...
							var = self.transitionnerMario(self.mario,300,self.monde.getSolAbsolu()-22,False)
							self.mario =  var  if not var  == None else self.mario
							#self.mario = BigMario(300,self.monde.getSolAbsolu()-22)
				...
				#GERE LES COLLISION DE LA BOULE DE FEU
				if isinstance(self.monde, Monde_3):
					for feu in self.monde.getBoss().getTabBoules():
						if feu.contactFeu(objet):
							self.monde.getBoss().getTabBoules().remove(feu)
			...
			for perso in self.monde.getTabPersonnages():
				...
				for boule in self.mario.getTabBoules():
					if not isinstance(perso,Mario):
						if isinstance(self.monde, Monde_3) and self.monde.getBoss().getVivant():
							if boule.contactBoule(self.monde.getBoss()):
								if not self.mario.getTabBoules() == []:
									self.mario.getTabBoules().remove(boule)
								self.monde.getBoss().setViesBoss(self.monde.getBoss().getViesBoss()-1)
								if self.monde.getBoss().getViesBoss() <= 0 :
									self.monde.getBoss().setVivant(False)
			...
			'''
			if isinstance(self.monde,Monde_3) and self.monde.getBoss().getVivant():
				if self.mario.contactAbsolu(self.mario,self.monde.getBoss()):
					self.mario.setMortFatale(True)
					self.mario.setSaut(True)
					self.mario.perdreVie(self.monde.getSolAbsolu(),self)

				self.monde.getBoss().deplacer(self.monde.getXPos(),self.monde.getDx())
				self.monde.getBoss().findMario(self.mario)
				if self.monde.getBoss().proche(self.mario):
					self.monde.getBoss().tirer(mario)
				self.gameFenetre.blit(pygame.image.load(self.monde.getBoss().sauter(self.monde.getHauteurPlafond(), self.monde.getYSol())),(self.monde.getBoss().getX(),self.monde.getBoss().getY()))

				#Verifier les collision de la boule de feu du boss et ses deplcameent
				for feu in self.monde.getBoss().getTabBoules():
					if feu.getX() < self.mario.getX() - 300 - feu.getLargeur():
						self.monde.getBoss().getTabBoules().remove(feu)
					else:
						if feu.getDirectionDroite():
							feu.setX(feu.getX() + 5)
						elif not feu.getDirectionDroite():
							feu.setX(feu.getX() - 5)
						feu.deplacer(self.monde.getXPos(),self.monde.getDx())
						if feu.contactFeu(self.mario):
							self.mario.setMortFatale(True)
							self.mario.setSaut(True)
							self.mario.perdreVie(self.monde.getSolAbsolu(),self)
						self.gameFenetre.blit(pygame.image.load(feu.getImgObjet()),(feu.getX(),feu.getY()))
